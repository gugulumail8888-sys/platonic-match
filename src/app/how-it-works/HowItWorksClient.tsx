...
const STEPS: Step[] = [
  ...
  },
  {
    ...
    title: '日程調整',
    icon: Calendar,
    description: '相手が承認したら希望日時を入力。日程確定後、前日までに両者それぞれ（無料プラン¥3,500・AIおすすめプラン¥3,000）をお支払いいただきます。事務局がGoogle Meetリンクをお送りします',
    badge: '各¥3,000〜3,500（税込）',
    variant: 'paid',
  },
  {
    number: 6,
    title: 'Google Meetお見合い',
    icon: Video,
    description: 'Google Meetでオンラインお見合い。顔を見ながらじっくりお話しましょう',
    badge: 'オンライン完結',
    variant: 'default',
  },
  {
...
const AI_STEPS: Step[] = [
  ...
  },
  {
    ...
    title: 'AIおすすめプランに申込',
    icon: Search,
    description: 'ログイン後、マイページ→設定タブから加入できます。キャンペーン期間は3ヶ月無料',
    badge: 'キャンペーン期間3ヶ月無料',
    variant: 'paid',
  },
  {
    number: 4,
    title: 'AIがあなたに合う相手を自動選定',
    icon: Search,
    description: '毎週AIがおすすめメンバーをご提案します',
    badge: '毎週更新',
    variant: 'default',
  },
  {
  ...
  },
  {
    ...
    title: '日程調整',
    icon: Calendar,
    description: '候補日を提案し合い日程を確定。確定後にお見合い料のお支払いが必要です',
    badge: '¥3,000（税込）',
    variant: 'paid',
  },
  {
  ...
  },
  {
    ...
    title: '交際希望ボタン',
    icon: ThumbsUp,
    description: 'お互いが交際を希望した場合のみ次に進みます',
    badge: '両者合意が必要',
    variant: 'default',
  },
  {
    ...
    title: '連絡先交換',
    icon: MessageCircle,
    description: '交際希望が一致した後に連絡先を交換できます',
    badge: 'ステップ完了',
    variant: 'default',
  },
  {
    number: 10,
    title: '解約はマイページからいつでも可能',
    icon: AlertTriangle,
    description: '設定タブからワンクリックで解約申請できます。違約金はありません',
    badge: '違約金なし',
    variant: 'default',
  },
  {
...
export default function HowItWorksClient() {
  ...
  const [activeTab, setActiveTab] = useState<'normal' | 'ai'>(
    searchParams.get('plan') === 'ai' ? 'ai' : 'normal'
  );
  const currentSteps = activeTab === 'normal' ? STEPS : AI_STEPS;
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      ...
      <div className="p-4 md:p-6 max-w-xl mx-auto">
        ...
        {searchParams.get('from') === 'top' && (
          <div className="mt-6 flex justify-center">
            <button
              onClick={() => {
                if (window.confirm('内容をご確認いただけましたか？よろしければログイン画面に進みます。')) {
                  router.push('/login');
                }
              }}
        ...
        <div className="mt-6 bg-zinc-800 rounded-2xl border border-zinc-700 p-5">
          ...
          <ul className="space-y-2">
            {[
              'お見合い費用（無料プラン¥3,500・AIおすすめプラン¥3,000）は日程確定後、前日までに両者それぞれにご請求します',
