      ...
      <div className="space-y-3">
        ...
        <ArticleSection number={5} title="お見合い申請料">
          <BulletList items={[
            ...
            '申請料は、お見合いの日程確定後にお支払いいただきます（申請時点・相手の承認前は費用は発生しません）',
            '日程確定・お支払い後のキャンセルは原則不可。キャンセルの場合、お支払い済みの申請料と同額のキャンセル料が発生します。ただし相手都合（無断キャンセル等、非がない側）の場合はお支払い済みの申請料を全額返金します',
            'キャンセルが多い場合には退会をお願いする場合があります',
        ...
        <ArticleSection number={6} title="月額オプション">
          <BulletList items={[
            'AIおすすめプランは月額料金が発生',
            '月額料金は980円（税込1,078円）',
            '解約は次回更新日の前日までに申請(更新日は契約開始日を基準とした毎月の周期です)',
          ]} />
        </ArticleSection>

        {/* 第7条 */}
        <ArticleSection number={7} title="退会・利用停止">
          <BulletList items={[
            'いつでも退会可能',
            '退会後、本人確認書類等の画像データは1年間保管後削除、その他の登録内容は3年間保管後削除',
            '退会後の返金不可',
            '会員が以下のいずれかに該当する場合、運営者は事前通知の上、会員資格を停止または取り消すことができます',
            '　・本規約(禁止事項等)に違反した場合',
            '　・登録情報に虚偽が判明した場合',
            '　・12ヶ月以上ログインがない場合(休眠会員)',
            '　・その他運営上必要と判断した場合',
            '休眠を理由とする資格取消しの場合、取消しの30日前までに登録メールアドレス宛に通知します',
            '本条に基づく利用停止・資格取消しについて、会員は異議を申し立てることはできません',
          ]} />
        </ArticleSection>
        ...
        {/* 第12条 */}
        <ArticleSection number={12} title="知的財産権">
          <BulletList items={[
            '本サービスに関するコンテンツ・商標・ロゴ等の知的財産権は運営者または正当な権利者に帰属します',
            '会員は運営者の事前の許諾なく、本サービスのコンテンツを複製・転載・改変・二次利用することはできません',
          ]} />
        </ArticleSection>

        {/* 第13条 */}
        <ArticleSection number={13} title="反社会的勢力の排除">
          <BulletList items={[
            '会員は、自己が暴力団・暴力団員・暴力団関係企業・総会屋その他反社会的勢力に該当しないことを表明し、将来にわたっても該当しないことを保証します',
            '前項に違反した場合、運営者は事前の通知なく会員資格を取り消すことができます',
          ]} />
        </ArticleSection>

        {/* 第14条 */}
        <ArticleSection number={14} title="サービスの中断・停止">
          <BulletList items={[
            'システムの保守点検、天災、通信回線の障害その他やむを得ない事由がある場合、運営者は会員への事前告知の上、または緊急の場合は告知なく本サービスの全部または一部を中断・停止することができます',
            '中断・停止により会員に生じた損害について、運営者は責任を負いません',
          ]} />
        </ArticleSection>

        {/* 第15条 */}
        <ArticleSection number={15} title="権利義務の譲渡禁止">
          <BulletList items={[
            '会員は、運営者の書面による事前の承諾なく、本規約に基づく地位または権利義務を第三者に譲渡・貸与・担保提供することはできません',
          ]} />
        </ArticleSection>

        {/* 第16条 */}
        <ArticleSection number={16} title="分離可能性">
          <BulletList items={[
            '本規約のいずれかの条項が法令により無効または執行不能と判断された場合であっても、その他の条項の効力には影響しません',
          ]} />
        </ArticleSection>

        {/* 第17条 */}
        <ArticleSection number={17} title="規約の存続">
          <BulletList items={[
            '退会後においても、第7条(データ保管に関する部分)・第8条(免責事項)・第9条(個人情報)・本条および第11条(準拠法・管轄)の規定はなお効力を有するものとします',
          ]} />
        </ArticleSection>
